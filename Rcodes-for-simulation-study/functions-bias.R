
# MC method to approximate integral
compute_MCest_TE2 <- function(dat,coefs,M=1e4)
{
  # convert coefs to the right dimension
  coefs <- matrix(coefs,ncol=1)
  require(mvtnorm)
  mu <- c(mean(dat$c1),mean(dat$c2))
  sd <- c(sd(dat$c1),sd(dat$c2))
  rho <- cor(dat$c1,dat$c2)
  Vmat <- matrix(NA,2,2)
  for(i in 1:ncol(Vmat)){
    for(j in 1:nrow(Vmat)){
      if(i==j){Vmat[i, j] <- sd[i]^2}
      if(i!=j){Vmat[i, j] <- rho*sd[i]*sd[j]}
    }
  }
  covs <- rmvnorm(n=M,mean = mu,sigma = Vmat)
  # covariate specific treatment effect
  # design matrix for the treatment
  covs_treat <- matrix(c(rep(1,M),rep(1,M),
                       covs,covs),byrow = FALSE,nrow = M)
  pE_MC <- ilogit(covs_treat%*%coefs)
  
  covs_control <- matrix(c(rep(1,M),rep(0,M),
                         covs,rep(rep(0,M),ncol(covs))),byrow = FALSE,nrow = M)
  pC_MC <- ilogit(covs_control%*%coefs)
  
  TEs_MC <- pE_MC - pC_MC
  
  # only integrate within the sensitive subgroup where E(pE|data) > E(pC|data)
  est_sens_size <- mean(TEs_MC>0)
  avg_TE_sens <- ifelse(est_sens_size==0,0,mean(TEs_MC[TEs_MC>0]))
  avg_TE_nonsens <- ifelse(est_sens_size==1,0,mean(TEs_MC[TEs_MC<=0]))
  # average treatment effect in the combined population
  avg_TE_combined <- mean(TEs_MC)
  return(c("est_TE_sens"=avg_TE_sens,
           "est_TE_nonsens"=avg_TE_nonsens,
           "est_TE_combined"=avg_TE_combined,
           "est_sens_size"=est_sens_size))
} 

compute_MCtrue_TE <- function(scenario,M=1e5)
{
  set.seed(86)
  require(mvtnorm)
  covs <- covs_gen(N=M,dat_description = scenario)
  covs2 <- response_gen(covs = covs,dat_description = scenario)
  TEs <- covs2$pE_true - covs2$pC_true
  sens_size <- mean(TEs>0)
  TE_sens <- ifelse(sens_size==0,0,mean(TEs[TEs>0]))
  TE_nonsens <- ifelse(sens_size==1,0,mean(TEs[TEs<=0]))
  TE_combined <- mean(TEs)
  return(c("true_TE_sens"=TE_sens,
           "true_TE_nonsens"=TE_nonsens,
           "true_TE_combined"=TE_combined,
           "true_sens_size"=sens_size))
}

compute_MCest_TE <- function(dat,post_coefs_mean,post_coefs)
{
  if(any(scenario$binary))
  {
    M <- nrow(dat)
    post_coefs_mean <- matrix(post_coefs_mean,ncol=1)
    covs <- as.matrix(dat,nrow = M)
    covs_treat <- matrix(c(rep(1,M),rep(1,M),
                           covs,covs),byrow = FALSE,nrow = M)
    pE_MC <- ilogit(covs_treat%*%post_coefs_mean)
    
    covs_control <- matrix(c(rep(1,M),rep(0,M),
                             covs,rep(rep(0,M),ncol(covs))),byrow = FALSE,nrow = M)
    pC_MC <- ilogit(covs_control%*%post_coefs_mean)
    dat$post_mean_te <- pE_MC - pC_MC
    dat$post_prob_benefit <- postprob_pE_gt_pC(covs,post_coefs)
    dat1 <- dat[dat$c1==1,]
    dat2 <- dat[dat$c1==0,]
    est_TE_sens <- mean(dat1$post_mean_te*dat1$post_prob_benefit)/mean(dat1$post_prob_benefit)+mean(dat2$post_mean_te*dat2$post_prob_benefit)/mean(dat2$post_prob_benefit)
    est_TE_combined <- mean(dat$c1)*mean(dat2$post_mean_te*dat2$post_prob_benefit)/mean(dat2$post_prob_benefit)+(1-mean(dat$c1))*mean(dat1$post_mean_te*dat1$post_prob_benefit)/mean(dat1$post_prob_benefit)
  } else{
  M <- nrow(dat)
  post_coefs_mean <- matrix(post_coefs_mean,ncol=1)
  covs <- as.matrix(dat,nrow = M)
  covs_treat <- matrix(c(rep(1,M),rep(1,M),
                         covs,covs),byrow = FALSE,nrow = M)
  pE_MC <- ilogit(covs_treat%*%post_coefs_mean)
  
  covs_control <- matrix(c(rep(1,M),rep(0,M),
                           covs,rep(rep(0,M),ncol(covs))),byrow = FALSE,nrow = M)
  pC_MC <- ilogit(covs_control%*%post_coefs_mean)
  dat$post_mean_te <- pE_MC - pC_MC
  dat$post_prob_benefit <- postprob_pE_gt_pC(covs,post_coefs)
  est_TE_sens <- mean(dat$post_mean_te*dat$post_prob_benefit)/mean(dat$post_prob_benefit)
  est_TE_combined <- mean(dat$post_mean_te)
  }
  return(c(est_TE_sens,est_TE_combined))
} 

