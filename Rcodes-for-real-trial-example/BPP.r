...
iters <- 1e3
c <- 1
# maximum tolerable sample size inflation for the enrichment block
sample_size_ratio <- 2
scenario <- scenario1
# record trial performance
JSD <- matrix(NA,nrow=iters,ncol=2)
colnames(JSD) <- c("treatment","control")
...
for(i in 1:iters)
{
 ...
 # mean(block1$response[block1$treatment==1])
  ...
  futility_prob_upper[i] <- beta.ineq(block1[block1$group==1,])
  ...
  res <- get_nextblock(type = "quantile",
                       ...
                       restriction = list(quantiles_est,prop_lower,sample_size_ratio))
  
  cutpoints <- c(cutpoints,quantiles_est[paste0((1-1/sample_size_ratio)*100,"%")])
...
save(JSD,opt_char,cohort_var_enrichment,est_TE,cutpoints,futility_prob_comb,futility_prob_upper,
    file=paste0("results_new/BPP-c",c,"-",scenario$`scenario name`,"-max",sample_size_ratio,".RData"))
